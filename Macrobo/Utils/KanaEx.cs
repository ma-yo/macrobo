...
namespace Macrobo.Utils
{
    ...
    public sealed class KanaEx
    {
        ...
        /// <returns></returns>
        public static string RomanToHiragana(string roman)
        {
            string[] roman2hiragana = new string[]{
              "a:あ", "i:い", "u:う", "e:え", "o:お",
            ...
            string result = "";
            roman = ToHankaku("" + roman).ToLower();
            string work = "";
            for (int i = 0; i < roman.Length; i++)
            {
                if (!Regex.IsMatch(roman.Substring(i, 1), @"[^a-zA-z]"))
                {
                    work += roman.Substring(i, 1);
                    bool exists = false;
                    foreach (var pair in roman2hiragana)
                    {
                    ...
                    if (!exists && work.Substring(0, 1) == "n" && work.Length > 1)
                    {
                        result += "ん";
                        work = work.Substring(1);
                    }
                    else if (!exists && work.Length == 2 && work[0] == work[1])
                    {
                        result += "っ";
                        work = work.Substring(1);
                    }
